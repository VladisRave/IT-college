...
**Декораторы** — это специальные функции, которые позволяют изменять или дополнять поведение других функций без изменения их исходного кода. Они действуют как «надстройка», оборачивая функцию дополнительной логикой. Такой подход помогает создавать чистый, гибкий и легко расширяемый код.

Декораторы удобны, когда требуется:

* добавлять логирование;
* измерять время выполнения функций;
* кэшировать результаты вычислений;
* контролировать права доступа;
* повторять выполнение при ошибках;
* автоматически обрабатывать входные данные.

### 3.2. Базовый синтаксис декоратора
...
### 3.3. Декораторы, работающие с аргументами функции
...
### 3.4. Декораторы с собственными параметрами

```python
def repeat(times):
    """Декоратор, повторяющий выполнение функции несколько раз."""
    def decorator(func):
        def wrapper(*args, **kwargs):
            results = []
...
### 3.6. Практическая польза декораторов

Декораторы помогают добавлять функциональность:
...
* повтор выполнения при ошибках;
* автоматическое преобразование типов.

### 3.7. Почему важен `functools.wraps`
...
`wraps` сохраняет имя, документацию и аннотации функции. Без него декорированная функция теряет метаданные.

### 3.8. Декораторы классов
...
### 3.9. Несколько декораторов для одной функции
...
Декораторы выполняются сверху вниз, а оборачивают функцию в обратном порядке.
